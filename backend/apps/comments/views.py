...
class CommentDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    # queryset параметр - загрудает один раз днные при запуске приложения - использовать для статических и редкоменяющихся даныхз
...
def post_comments(request, post_id):
    ...
    # ищем опсти и если ео нет выводим ошибку
...
def comment_replies(request, comment_id):
    ...
    parent_comment = get_object_or_404(Comment, id=comment_id, is_active=True) # сам комментарий на который ответы нжуны
    
    # ищщем все ответы на данный коммент
    replies = Comment.objects.filter(
        parent=parent_comment,
        is_active=True
    ).select_related('author').order_by('created_at')
    # валидированные данные ответа к комменту
