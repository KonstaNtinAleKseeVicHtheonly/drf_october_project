...
class Category(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug: # если слаг не создался то сделаем его
...
class Post(models.Model):
    ...
    category = models.ForeignKey(
        Category,
        on_delete=models.SET_NULL, # категорию нельзя удлаить пока существуют посты в ней
        null=True,
        blank=True,
        related_name='posts'
    )
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, # из settings проекта AUTH_USER_MODEL = 'accounts.User'
        on_delete=models.CASCADE, # при удалении юзера - удалятся все его посты
        related_name='posts'
    )
    status = models.CharField( # чекбокс с статусом
        max_length=10,
        choices=STATUS_CHOICES,
        default='published'
    ) # либо черновое Drafted, либо опубликвоано Published
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    views_count = models.PositiveIntegerField(default=0) # счетсик просмотров у поста
    ...
    class Meta:
        ...
        ] # поля по которым будет быстрее поиск

    def __str__(self):
        return self.title

    def save(self, *args, **kwargs): # сохраяем слаг от name
        if not self.slug:
            self.slug = slugify(self.title)
        super().save(*args, **kwargs)

    def get_absolute_url(self): # в дальнейшем по id поста можно получить к нему полную ссылку на сайте 
        #reverse() — генерирует URL по имени маршрута и параметрам
    ...
    @property
    def pinned_posts(self):
        """Информация о закреплении поста"""
        if hasattr(self, 'pin_info') and self.pin_info:
            return {
                'pinned_at': self.pin_info.pinned_at,
                'pinned_by': self.pin_info.user.username if self.pin_info.user else None,
                'is_active': True
            }
        return None
    ...
    def increment_views(self):
        """Увеличивает счетчик просмотров (каждый раз когд кто то заходит на пост)"""
        self.views_count += 1
